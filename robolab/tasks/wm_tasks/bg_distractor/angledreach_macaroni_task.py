...
STATUS_PATH = Path(ASSET_DIR) / "wm_tasks" / "AngledReachMacaroniTask" / "status.json"


@configclass
class AngledReachMacaroniTerminations:
...
@dataclass
class AngledReachMacaroniTask(Task):
    contact_object_list = ["table", "macaroni_and_cheese", "canned_tuna", "popcorn_box"]
    scene = import_scene("macaroni_center_table.usda", contact_object_list)
    terminations = AngledReachMacaroniTerminations
    instruction = {
        "default": "AngledReachMacaroni",
